...
function getPostsByTopic($phpSrcx,$topic_id){
	include('config.php');
	include('kittenconfig.php');
	$con = mysql_connect($dbhost, $dbuser,$dbpasswd);
	mysql_select_db($phpTarget);
	$query = "SELECT * FROM ".$phpSrcx.".phpbb_posts where topic_id ==$topic_id";
	$result = mysql_query($query )
	or die(mysql_error());
	$row = mysql_fetch_array(  $result );
	return $row;
...
$query = "select * from thekitt_phpb2.phpbb_topics a where   a.forum_id = 343 order by a.topic_title ;" ;
...
while($row = mysql_fetch_array(  $result )){
	...
	if (isset($topicList[$topicName])){
	
	
		echo "Name".$topicName." Old :".$topicList[$topicName]." New:".$row['topic_id']."\n";
		$queryOut ="delete from thekitt_phpb2.phpbb_posts where topic_id=".$row['topic_id'].";";
		echo $queryOut."\n";
		$result1 = mysql_query($queryOut )
		or die(mysql_error());
		$queryClose="delete from thekitt_phpb2.phpbb_topics where topic_id=".$row['topic_id'].";";
		echo $queryClose."\n";
		$result2 = mysql_query($queryClose )
	or die(mysql_error());
	
	}else
	{
	$topicId = $row['topic_id'];
	$topicList[$topicName] = $topicId ;
	}
}
