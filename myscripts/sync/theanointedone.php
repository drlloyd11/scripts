...
while ($run == 1){
	#echo $argc."-\n";
	if ($argc >1){
		$query = "SELECT * FROM ".$phpSrc.".phpbb_users where user_id =".$argv[1].";\n";
		$run  =0;
	}
	else
	{
		$query = "SELECT * FROM ".$phpSrc.".phpbb_users where user_id >=$postCount and user_id <$postStop  order by user_id "  ;
	}
	...
	//	$newPost = $row['post_text'];
		...
		//if (($argc >1 ) ){
			$newPost =scrubber($newPost);
		//}
		echo "user id".$newPost."\n";
		$newQuery = sprintf("UPDATE ".$phpTarget.".phpbb_users SET user_sig = '%s' where user_id= '%s'", mysql_real_escape_string($newPost), $userID);
		$val  =1;
		#echo "\n===========================\n";
			#echo $newQuery;
		#echo "\n=============================\n";
...
function ez2bbcode($s) {
	$ez2tags= array(
			...
			'/(<!--EZCODE FONT START-->)/is',#9
			'/(<!--EZCODE FONT END-->)/is',#10
			'/(<!--EZCODE UNDERLINE START-->)/is',#9
			'/(<!--EZCODE UNDERLINE END-->)/is',#10
